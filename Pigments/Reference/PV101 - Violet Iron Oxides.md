---
title: PV101 - Violet Iron Oxides
swatchColor: "#5E2A4E"
sequence: 57
---
<div style="float:left;width:64px;height:64px;background:#5E2A4E;margin:4px 16px 8px 0;border-radius:2px;"></div>

> Color swatch: closest approximation of the pigment's hue (not measured spectral data — see [[Pigments/Sourcing Real Spectral Data|Sourcing Real Spectral Data]]).

**Scope:** Violet Iron Oxides (PV101 family) — protocrystalline hematite grown on clay, the furnace-made culminations of the sienna chemistry.

### Identity

- **Colour Index:** PV101 (and bluish Indian reds within PR101); **Mars violet and the bluish varieties of Indian red** (*Artists' Pigments*).
- Often produced from **protocrystalline hematite associated with clay minerals**; certain iron silicates (e.g., **celadonite**) possess a **micaceous, layered structure** (*Artists' Pigments*, *Color for Science, Art, and Technology*).

### Production

- **Heat treatment of sienna clays** (which contain iron and manganese): natural yellow-brown of raw sienna → warm reddish-brown → with further heat, **maroon or violet-pink tones** (*Artists' Pigments*).
- Violet shades require **high furnace temperatures or specific precipitation methods** creating larger, more uniform particle sizes — the hematite base then reflects light more toward the **violet (blue-red) end** of the spectrum (*The Science of Paintings*).

### Color Data

- Part of the "culmination of chemical excitation" of the earth palette; reflects more blue-red than any natural red earth (*The Science of Paintings*).
- Shared PBr7/PV101 durability: absolutely lightfast; chemically pure when synthetic (*Artists' Pigments*).
- **Munsell notation:** not established in this corpus for the violet oxides.

### The hematite family map (hydration state → hue)

| Pigment | Mineral | State | Hue |
|---|---|---|---|
| Raw sienna | goethite FeO(OH), hydrated + clay + Mn | unheated | yellow-brown |
| Burnt sienna | hematite Fe₂O₃ (dehydrated goethite) | calcined | transparent fiery reddish-brown |
| Red ochre | natural hematite | native (or yellow ochre "increased by fire") | red |
| Violet oxides | protocrystalline hematite on clay minerals | high temperature / controlled precipitation | violet-pink to maroon |

The whole earth palette is governed by **hydration state and calcination temperature** of iron ("yellow ochre can be increased by fire to become red"; burnt sienna is goethite dehydrated; violet is hematite refined further) (*Artists' Pigments*, *Goethe's Theory of Colours*, *The Science of Paintings*).

### Relation to Art and Painters

- Mars violet and Indian red give the painter a **deep blue-red** that natural earths cannot reach without gazing into synthetic territory — the "broken color" shadow-tones of portraiture and late 19th-century painting (see [[Painting/Movements & Painters/Post-Impressionism|Post-Impressionism]]).

## Handprint Perspectives

MacEvoy treats the violet oxide in his palette structure as the **deepest red included in the neutral-red range**: a granulating or semi-transparent earth used in shadow mixtures and to warm darks, complementing ultramarine — his "utility" dark combinations commonly use the transparent red-browns and violets of the earth family rather than black. His attribute charts place these on the opaque-to-semi-opaque tier with fine, even grain *(Source: [[raw_sources/handprint/pigmt3.md|pigmt3.html]], [[raw_sources/handprint/palette1.md|palette1.html]])*.

## Subtopics
- PV101; protocrystalline hematite on clay; celadonite mica-like structure
- Sienna clays roasted to maroon; high-temperature precipitation routes
- Larger uniform particles reflect violet; "chemical excitation" endpoint
- The goethite → hematite hydration ladder (table above)
- Deep blue-red niche in shadows and broken color

## Cross-References
- [[Pigments/index|Pigments]] — the section hub
- [[Pigments/Reference/PR101 - Mars Red|PR101 - Mars Red]] — the red hematite grade
- [[Pigments/Reference/PBr7 - Burnt Sienna|PBr7 - Burnt Sienna]] — the roasted earth
- [[Pigments/Chemistry|Chemistry]] — polymorphs and hydration
- [[Painting/Movements & Painters/Post-Impressionism|Post-Impressionism]] — broken-color palette

## Sources
* "Artists' Pigments: A Handbook of Their History and Characteristics" — Robert L. Feller
* "The Science of Paintings" — W. Stanley Taft Jr.
* "Color for Science, Art, and Technology"
* "Theory of Colours" — Johann Wolfgang von Goethe