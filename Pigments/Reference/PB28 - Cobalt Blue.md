---
title: PB28 - Cobalt Blue
swatchColor: "#2B4FA3"
sequence: 38
---
<div style="float:left;width:64px;height:64px;background:#2B4FA3;margin:4px 16px 8px 0;border-radius:2px;"></div>

> Color swatch: closest approximation of the pigment's hue (not measured spectral data — see [[Pigments/Sourcing Real Spectral Data|Sourcing Real Spectral Data]]).

**Scope:** Cobalt Blue (PB28) — Thenard's blue, the aluminate standard that replaced smalt and became the modern blue for skies.

### Identity

- **Colour Index:** PB28; **cobalt aluminate, CoO·Al₂O₃** — cobalt oxide + aluminum oxide; often called **"Thenard's blue"**.
- A bright, clear blue with a noticeable **violet (reddish) undertone**.

### History

- Discovered in **1802 by the French chemist Louis-Jacques Thénard**; entered artistic use by ~**1807**; commercially available as an artist's color between **1820 and 1830**.
- Replaced the coarser **smalt** (cobalt glass) as the modern standard permanent blue.

### Production

**High-temperature calcination of cobalt compounds with alumina**.

### Color Data

- **Transparency/strength:** semi-transparent; **moderate tinting strength, comparable to ultramarine**.
- **Permanence:** lightfast and permanent for all uses; in oil an **average to slow drier**.
- **Munsell notation:** not established in this corpus for PB28.

### Relation to Art and Painters

- Used extensively by **Renoir and Monet** in their "cunning play of modulations" of cold and warm colors; associated with **J.M.W. Turner**, who used delicately graduated blues to controvert the academic "doctrine of the balance of colors" favoring warm tonalities.

## Handprint Perspectives

MacEvoy's palette treats cobalt blue as the **semi-transparent, slightly violet-leaning** sky-and-distance blue — the "utility" alternative to ultramarine when a lighter, less warm blue is wanted; in watercolor it sits in the non-granulating, mild-staining middle tier, and his color-mixing doctrine uses it in aerial-perspective pairs with cerulean. He flags that its moderate strength means it cannot carry heavy darks alone *(Source: [[raw_sources/handprint/palette1.md|palette1.html]], [[raw_sources/handprint/pigmt3.md|pigmt3.html]])*.

## Subtopics
- CoO·Al₂O₃; Thenard's blue
- Thénard 1802; artistic use ~1807; commercial 1820s–30s
- High-temperature calcination with alumina
- Semi-transparent, moderate strength, lightfast, slow oil drier
- Replaced smalt; Renoir/Monet modulations; Turner's graduated blues

## Cross-References
- [[Pigments/index|Pigments]] — the section hub
- [[Pigments/Reference/PB35 - Cerulean Blue|PB35 - Cerulean Blue]] — pale greenish companion
- [[Pigments/Reference/PB29 - Ultramarine Blue|PB29 - Ultramarine Blue]] — the warmer deep blue
- [[Painting/Movements & Painters/Impressionism|Impressionism]] — cold/warm modulations

## Sources
* "Artists' Pigments: A Handbook of Their History and Characteristics" — Robert L. Feller
* "The Science of Paintings" — W. Stanley Taft Jr.
* "Color in Art" — John Gage
* "Ralph Mayer — The Artist's Handbook of Materials and Techniques" — Ralph Mayer
* "Interaction of Color" — Josef Albers
* "Claude Monet — The Alchemist of Light"