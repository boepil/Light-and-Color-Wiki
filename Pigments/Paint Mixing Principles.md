---
title: Paint Mixing Principles
sequence: 22
---
![[Pasted image 20260816111729.png]]

**Scope:** The core physical, optical, and chemical principles governing subtractive paint mixtures in the studio, including handling practices, drying shifts, and palette organization systems.

### 1. The Physics of Subtractive Accumulation

Every time an artist mixes two or more paints, they are performing **[[Intersections/Optical vs. Physical Mixture|subtractive color mixture]]**. Unlike additive mixing (light), which increases luminance by combining wavelengths, pigments function by absorbing light. 

*   **The Accumulative Subtraction Law:** Each added pigment acts as a spectral filter, absorbing specific bands of the visible spectrum. When multiple pigments are blended, their absorption profiles accumulate. This reduces the total reflected light energy, causing the mixture to systematically drop in both lightness (value) and saturation (chroma).
*   **The Genesis of "Mud":** "Muddy" mixtures—characterized by a lifeless, desaturated brownish-gray appearance—are the natural result of over-mixing. When too many pigments are combined, or when warm and cool pigments with mismatched reflectance peaks are blended, nearly the entire visible spectrum is absorbed, flattening the reflectance curve. To maintain clean, high-chroma mixtures, artists must minimize the number of components in a blend.
*   **The Three-Paint Rule:** A fundamental studio guideline is to limit any single mixture to a maximum of three paints (excluding white). While three paints allow the artist to navigate hue, value, and chroma coordinates, adding a fourth paint introduces chemical and optical variables that make the mixture behave unpredictably, leading to rapid desaturation.

### 2. Tinting Strength and Pigment Density Disparities

A common point of failure in paint mixing is failing to account for the massive differences in **tinting strength** and density between pigment families.

*   **High-Chroma Synthetics vs. Mineral Earths:** Modern organic synthetic pigments (such as **[[PB15 - Phthalo Blue|Phthalo Blue]]** PB15 or **[[PR122 - Quinacridone Magenta|Quinacridone Magenta]]** PR122) possess extremely high tinting strengths due to their small particle size and high molar extinction coefficients. In contrast, inorganic earth pigments (such as **[[PBr7 - Burnt Sienna|Burnt Sienna]]** PBr7 or **[[PY43 - Yellow Ochre|Yellow Ochre]]** PY43) have low tinting strengths and large, dense particles.
*   **Volumetric Proportionality:** If an artist mixes equal parts of Phthalo Blue and Burnt Sienna, the phthalo will instantly overwhelm the earth pigment, resulting in a dark blue wash. To achieve neutralization, the artist must add the stronger pigment to the weaker pigment in minuscule increments (the "drop-by-drop" rule).
*   **Inorganic Settling (The Flocculation Trap):** Liquid mixtures of heavy inorganic mineral pigments (like cadmiums or cobalts) and light organic pigments (like phthalos) are physically unstable. The heavy minerals sink quickly to the bottom of the paint puddle, making the wet mix look cooler than it will appear when brushed out on paper.

### 3. Drying Shifts and Binder Refractive Index

A paint mixture changes its color appearance as it dries. The magnitude and direction of this **drying shift** depend on the medium's binder and the changing refractive index ($n$) of the film.

| Medium | Wet Binder Refractive Index | Dry Film Refractive Index | Typical Drying Shift |
| :--- | :--- | :--- | :--- |
| **Watercolor** | Water ($n \approx 1.33$) | Air ($n = 1.00$) | Dries **lighter** and **less saturated** (loses 20–30% chroma/value). |
| **Acrylic** | Acrylic emulsion (milky, $n \approx 1.40$) | Dry polymer (clear, $n \approx 1.48$) | Dries **darker** and **more saturated**. |
| **Oil** | Wet linseed oil ($n \approx 1.48$) | Oxidized oil film ($n \approx 1.48$) | Minimal shift; dries slightly darker over time as oil yellows. |

In watercolor, the evaporation of water leaves the pigment particles surrounded by air ($n=1.00$). The large refractive index difference between air and pigment ($n \approx 1.5\text{--}2.7$) causes intense light scattering at the boundary, making the dried paint layer reflect more white surface light, which desaturates the color. In acrylic, the binder shifts from a milky white emulsion to a clear polymer film, which increases light transmission into the pigment layers, darkening the paint.

### 4. Opacity, Glazing, and Paint Layering

Understanding the hierarchy of transparency and opacity is crucial for successful paint application.

*   **Transparency/Opacity Dynamics:** Opaque pigments (such as **[[PW6 - Titanium White|Titanium White]]** PW6 or **[[PR108 - Cadmium Red|Cadmium Red]]** PR108) scatter light heavily before it can penetrate deep into the paint film. Transparent pigments (such as [[High-Chroma Synthetics|Quinacridone Rose]] PV19 or **[[PB15 - Phthalo Blue|Phthalo Blue]]** PB15) allow light to pass through, reflecting instead off the underlying ground or paper.
*   **Glazing vs. Scumbling:** A **glaze** is a thin, transparent layer of paint applied over a dry, lighter color. The light passes through the glaze, bounces off the background, and passes back through the glaze, creating rich, luminous depth. A **scumble** is a thin, opaque, or semi-opaque light layer applied over a darker passage, scattering light to create a cool, hazy, or smoky effect (similar to **[[Natural Daylight Variation & Hyperspectral Scene Data|Rayleigh scattering]]**).
*   **Shadow and Light Placement:** As a rule, shadows should be kept thin and transparent to suggest depth and atmosphere, while highlights and lit areas should be painted opaquely to create physical weight and presence.

### 5. Practical Palette Systems and Harmony

To maintain control over color mixing, artists employ structured palette organization systems:

*   **Limited Palettes:** Using a small, dedicated set of paints (typically 3 to 6) ensures that all mixed colors share common chemical ancestors, creating automatic color harmony.
*   **The Zorn Palette:** A famous historical limited palette consisting of only four colors: **[[PY43 - Yellow Ochre|Yellow Ochre]]** (PY43), Vermilion (or **[[PR108 - Cadmium Red|Cadmium Red]]** PR108), Ivory Black (PBk9), and **[[PW6 - Titanium White|Titanium White]]** (PW6). The Zorn palette is highly effective for figure painting, as Ivory Black acts as a cool blue complement when mixed with white, allowing the artist to mix clean skin tones, warm shadows, and muted olive greens without a true blue paint.
*   **The Mother Color Technique:** To unify a chaotic color scheme, the artist mixes a tiny touch of a single dominant "mother color" (such as a warm amber or a cool violet) into every paint mixture across the entire canvas, creating a shared color envelope or ambient light sensation.

---

## Handprint Perspectives

Bruce MacEvoy emphasizes that color mixing rules are often oversimplified by art schools. He notes:
1.  **The "Split-Primary" Myth:** The split-primary system (having a "warm" and "cool" tube of each primary) is a useful training exercise but fails to account for the unique physical properties of actual pigments (granulation, tinting strength, density), which behave like raw chemicals rather than abstract points on a wheel *(Source: [[raw_sources/handprint/mix.md|mix.html]])*.
2.  **Visual vs. Mixing Complements:** Painters must distinguish between visual complements (which create high-contrast simultaneous contrast) and mixing complements (specific paint pairs that neutralize to gray). They do not always align on a standard color wheel *(Source: [[raw_sources/handprint/mixtable.md|mixtable.html]])*.
3.  **The 3-Paint Rule Utility:** Keeping mixtures to a maximum of three paints prevents the accumulation of impurities and minimizes drying shifts, which are notoriously difficult to predict when combining multiple brands *(Source: [[raw_sources/handprint/mix.md|mix.html]])*.

> [!WARNING] **Contradiction Flag: The Primary Triad Fallacy**
> Standard art instruction claims that three primaries (RYB or CMY) can mix "every color." However, MacEvoy's spectrophotometric gamut maps show that any three-pigment triad mixes a lopsided, restricted gamut. A three-primary palette always sacrifices chroma in secondary mixtures (like mixing a dull purple from blue and red). True studio utility requires a multi-pigment palette (usually 6 to 12 paints) to cover the perimeter of the human visual gamut *(Source: [[raw_sources/handprint/mix.md|mix.html]])*.

---

## Subtopics

*   **Refractive Index (RI) matching:** The physics of how the refractive index gap between pigment and binder determines transparency.
*   **Zorn Palette Mixtures:** The specific chromatic pathways that allow a red-yellow-black-white palette to mix skin tones and olive greens.
*   **Rayleigh Scattering in Paint:** The optical physics behind why thin scumbles of light paint over dark grounds appear blue.

---

## Cross-References

*   [[Pigments/Two-Pigment Neutrals|Two-Pigment Neutrals]] — Mixing complements and neutralizing paint pairs.
*   [[Pigments/Transparency, Opacity & Pigment Codes|Transparency, Opacity & Pigment Codes]] — Label codes and the physics of light scattering.
*   [[Pigments/Media, Vehicles & Solvents|Media, Vehicles & Solvents]] — Refractive index values of different binders.
*   [[Intersections/Color Mixing, Prediction & Color Schemes|Color Mixing, Prediction & Color Schemes]] — Multiplicative filter math and Zorn palette geometry.
*   [[Colors/Why CMY Beats RYB for Color Mixing|Why CMY Beats RYB for Color Mixing]] — The spectral physics of primary selection.

---

## Sources

*   "The Artist's Handbook of Materials and Techniques" — Ralph Mayer
*   "Carlson's Guide to Landscape Painting" — John F. Carlson
*   "The Dimensions of Colour" — David Briggs
*   "handprint : color mixing" — Bruce MacEvoy
*   "Interaction of Color" — Josef Albers
