...
MacEvoy's paint-chemistry pages read like a field manual for exactly these mechanisms. He states the transparent-vehicle law as a physical rule — "the proportion of black absorptance is increased by a transparent vehicle," and glossy surfaces reduce white-light scattering — which is why "pigments in acrylic and oil paints can achieve a higher chroma than the same pigments in watercolors," where the gum vehicle has dried around the paper fibers and sunk into the pulp. His paint-physics list ties the same properties the corpus cites (refractive index, particle size, pigment load, vehicle purity) to practical handling: tinting strength is set by how much the vehicle dilutes the pigment, staining vs. lifting follows from gum proportion and particle size, and the traditional vehicle itself — "traditionally gum arabic but now often a synthetic carbohydrate" — is a variable the artist chooses, not a given. On gouache he is the corpus's most exact voice: the gum-arabic vehicle with isinglass (fish gelatin) or animal size as the historical alternative, chalk/whiting opacity and the matte sheen, the cracking from thick layering, and the caution that gouache must not be mixed with acrylic chemicals, which can change hue or turn lumpy *(Source: [[raw_sources/handprint/color18a.md|color18a.html]], [[raw_sources/handprint/intstud.md|intstud.html]], [[raw_sources/handprint/pigmt7.md|pigmt7.html]], [[raw_sources/handprint/pigmt5.md|pigmt5.html]], [[raw_sources/handprint/color16.md|color16.html]])*.

## HueValueChroma Perspectives

Briggs grounds the "same pigment, different color" phenomenon in the **medium's control of which mixing physics dominates**, not just the refractive-index gap. The three ways an artist combines colour carry different weights of subtractive vs. additive-averaging mixing, and the **vehicle shifted the balance**: when the same pigment grains are *not* enclosed in a medium — dry powders blended in pastel — "the mixing path is visually closer to the additive-averaging path," because light bounces off individual grains; the moment the grains are enclosed (oil, acrylic), the medium's higher index and internal reflection "make the pigment grains more transparent" and multiply bounce light between grains, "two factors that favour the subtractive component" *(Source: [[raw_sources/huevaluechroma/061.md|061.html]])*. That is the same mechanism behind the page's oil-darkens / watercolor-lights rule, stated in terms of *mixing behaviour* rather than reflectance:

- **Glazing is subtractive; lifting is a real alternative to tinting with white.** Glazed transparent layers mix subtractively, and — principle 2 of 6.3 — a transparent colour *glazed over white attains a higher maximum chroma than the same colour physically mixed with white paint*, because layering keeps scattering light through the pure colourant instead of diluting it with white particles *(Source: [[raw_sources/huevaluechroma/063.md|063.html]])*. This is the technical reason thin watercolor/gouache washes over paper can surpass the chroma of the same pigment lightened with white body.
...
- Egg tempera (yolk emulsion, brittle), casein (milk protein, water-insoluble), pastel (dry binderless pigment)
...
- [[Natural vs. Synthetic]] — the materials these vehicles carry
- [[Reflection vs. Emission]] — the reflection physics behind the gloss rules
- [[Optical vs. Physical Mixture]] — how medium changes mixture behavior
