---
title: Bibliography
sequence: 3
---

**Scope:** The unified citation map of this wiki — every book, paper, video, and web resource used across the project. Sources are listed by title and author; page-level citations appear on the individual wiki pages where they are used.

## Light

* "Color and Light in Nature" — Lynch & Livingston
* "Light and Colour in the Outdoors" — Minnaert
* "Color for Science, Art, and Technology" — Kurt Nassau (Editor)
* "Theory of Colours" — Johann Wolfgang von Goethe
* "The Science of Paintings" — W. Stanley Taft Jr. and James W. Mayer

## Eye

* "Vision Science: Photons to Phenomenology" — Stephen E. Palmer
* "Color Vision: From Genes to Perception" — Gegenfurtner & Sharpe (Editors)
* "Illusions of Seeing" — Thomas Ditzinger
* "Contemporary Color: Theory and Use" — Steven Bleicher
* "Color Vision: Trichromatic and Opponent Process Theories" — Intro Psych Tutorial (video)
* "A Comprehensive Overview of Color Vision Mechanisms, Color Spaces, and the Munsell System" — NotebookLM source
* "Illustrating Color Evolution and Color Blindness" — NotebookLM source
* "The Neuroscience Behind Why Colours "Rewire" Your Brain" — NotebookLM source
* "Why Material Reality Favors Green Over Red: The Physical Chemistry of Chromatic Limits" — NotebookLM source
* "The Material Supremacy of Green Chroma" — NotebookLM source

## Colors

* "Color Science" — Wyszecki & Stiles
* "Measuring Colour" — Hunt
* "Color Appearance Models" — Fairchild
* "Color Management: A Comprehensive Guide for Graphic Designers" — John T. Drew and Sarah A. Meyer
* "A Color Notation" — Albert Henry Munsell
* "Munsell Renotation Table" — NotebookLM source
* "Munsell Color Science Lab Educational Resources" — Rochester Institute of Technology
* "Ordering Colour: Albert Henry Munsell (1858-1918)" — The Eclectic Light Company
* "Where did the Warm/Cool Thing Come From? (Color Theory History)" — YouTube source

## Painting

* "The Principles of Harmony and Contrast of Colours" — Chevreul
* "Interaction of Color" — Josef Albers
* "The Art of Color: The Subjective Experience and Objective Rationale of Color" — Johannes Itten
* "Colour and Culture" — John Gage
* "Color and Meaning" — John Gage
* "Colour in Art" — John Gage
* "Chromophobia" — Batchelor
* "Bright Earth" — Philip Ball
* "Color: A Natural History of the Palette" — Victoria Finlay
* "The Artist's Handbook of Materials and Techniques" — Ralph Mayer
* "Pigment Compendium" — Eastaugh et al.
* "Artists' Pigments: A Handbook of Their History and Characteristics" — Robert L. Feller (Editor)
* "Carlson's Guide to Landscape Painting" — John F. Carlson
* "Chinese Ink Painting: Techniques, Materials, and Concepts" — Jean Long
* "Color Mixing the Van Wyk Way: A Manual for Oil Painters" — Helen van Wyk
* "Color by Betty Edwards: A Course in Mastering the Art of Mixing Colors" — Betty Edwards
* "Composition of Outdoor Painting" — Edgar Alwin Payne
* "Concept Art Accelerator" — Walid Feghali
* "Courbet and the Modern Landscape" — Mary Morton and Charlotte Eyerman
* "David Bellamy's Watercolour Landscape Course" — David Bellamy
* "Devin Korwin: Creative Fundamentals" — Devin Korwin
* "Imaginative Realism: How to Paint What Doesn't Exist" — James Gurney
* "Mastering Atmosphere & Mood in Watercolor" — Joseph Zbukvic
* "Modern Art: A Very Short Introduction" — David Cottington
* "Plein Air Techniques for Artists" — Aimee Erickson
* "The Case Against Color Bias" — Color Nerd (web/video source)
* "The Simple Secret to Better Painting" — Greg Albert
* "The Student's Guide to Painting" — Jack Faragasso
* "The Art of Painting in Oil, and in Fresco" — Jean-François-Léonor Mérimée

## Handprint (web sources)

Handprint (handprint.com) is Bruce MacEvoy's color theory and watercolor resource. Raw copies live in `raw_sources/handprint/`.

* "handprint : tonal value" — [[raw_sources/handprint/color11.md|color11.html]]
* "handprint : color temperature" — [[raw_sources/handprint/color12.md|color12.html]]
* "handprint : color wheels" — [[raw_sources/handprint/color13.md|color13.html]]
* "handprint : color wheels (II)" — [[raw_sources/handprint/color14.md|color14.html]]
* "handprint : an artist's color wheel" — [[raw_sources/handprint/color16.md|color16.html]]
* "handprint : modern color theory (concepts)" — [[raw_sources/handprint/color18a.md|color18a.html]]
* "handprint : modern color theory (applications)" — [[raw_sources/handprint/color18b.md|color18b.html]]
* "handprint : learning color through paints" — [[raw_sources/handprint/intstud.md|intstud.html]]
* "handprint : 2004 lightfastness tests" — [[raw_sources/handprint/litetest.md|litetest.html]]
* "handprint : basic mixing method" — [[raw_sources/handprint/mix.md|mix.html]]
* "handprint : watercolor mixing complements" — [[raw_sources/handprint/mixtable.md|mixtable.html]]
* "handprint : the complete palette" — [[raw_sources/handprint/palette1.md|palette1.html]]
* "handprint : how to choose watercolor paints" — [[raw_sources/handprint/pigmt0.md|pigmt0.html]]
* "handprint : how watercolor paints are made" — [[raw_sources/handprint/pigmt1.md|pigmt1.html]]
* "handprint : watercolor brands" — [[raw_sources/handprint/pigmt2.md|pigmt2.html]]
* "handprint : the material attributes of paints" — [[raw_sources/handprint/pigmt3.md|pigmt3.html]]
* "handprint : interference paints" — [[raw_sources/handprint/pigmt4.md|pigmt4.html]]
* "handprint : tube, pan & liquid watercolors" — [[raw_sources/handprint/pigmt5.md|pigmt5.html]]
* "handprint : labeling, lightfastness & toxicity" — [[raw_sources/handprint/pigmt6.md|pigmt6.html]]
* "handprint : gouache & bodycolor" — [[raw_sources/handprint/pigmt7.md|pigmt7.html]]
* "handprint : what the ratings mean" — [[raw_sources/handprint/pigmt8.md|pigmt8.html]]
* "handprint : doing your own lightfastness tests" — [[raw_sources/handprint/pigmt9.md|pigmt9.html]]
* "handprint : color harmony & design" — [[raw_sources/handprint/tech13.md|tech13.html]]
* "handprint : watercolor fundamentals" — [[raw_sources/handprint/waterfs.md|waterfs.html]]

## Cross-References

- [[History & Key Figures]]
