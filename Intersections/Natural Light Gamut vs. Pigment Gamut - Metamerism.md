> **Header image prompt (flat medical-textbook blue anatomy-plate style, wide banner, 16:9 proportion):** A monochrome blue line-art diagram on a pale blue background of natural light versus pigment limits. Center: a large horseshoe **SPECTRUM LOCUS / CIE xy** containing a wide irregular blob labeled **NATURAL LIGHT GAMUT** and, nested inside, the lopsided **PIGMENT GAMUT** (green-yellow bulge). Left: two small SPD graphs — a jagged natural-scene curve **NATURAL SPD** and a rectangular **0–100% PIGMENT REFLECTANCE** — with a caption **REFLECTANCE BOUND 0–100%**. Right: a metamerism panel showing two different SPD curves (a monochromatic **580 nm** peak and a **RED+GREEN** two-peak curve) converging through three cone icons labeled **S · M · L** into one identical patch labeled **LOOK THE SAME (METAMERS)**, with a lamp icon labeled **ILLUMINANT CHANGE → MISMATCH**. Bold uppercase labels with thin leader lines and arrowheads, flat vintage anatomy-plate look, no realism, no 3D, no watermark, no color beyond blue tones.
